...
void latexStyleOutput(Factors *factors, int order) {
    ...
    for (int i = order; i >= 0; --i) {
        ...
        if (factors->sequence[i][1] == factors->sequence[i][0] || factors->sequence[i][1] == 1) {
            ...
            if (factors->sequence[i][0] * factors->sequence[i][1] > 0) {
                fprintf(output, "\\frac{%lld}{%lld}",
                        factors->sequence[i][0] > 0 ? factors->sequence[i][0] : -factors->sequence[i][0],
                        factors->sequence[i][1] > 0 ? factors->sequence[i][1] : -factors->sequence[i][1]);
            } else {
                fprintf(output, "-\\frac{%lld}{%lld}",
                        factors->sequence[i][0] > 0 ? factors->sequence[i][0] : -factors->sequence[i][0],
                        factors->sequence[i][1] > 0 ? factors->sequence[i][1] : -factors->sequence[i][1]);
